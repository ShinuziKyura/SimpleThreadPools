...
namespace stp // C++14 version
{
	...
	public:
		...
		void new_task(task<RetType> & task, task_priority priority = task_priority::normal)
		{
			...
			{
				std::lock_guard<std::shared_mutex> lock(_threadpool_mutex);
		...
		void delete_tasks()
		{
			std::lock(_threadpool_mutex, _task_queue_mutex);
			std::lock_guard<std::shared_mutex> lock_1(_threadpool_mutex, std::adopt_lock);
			std::lock_guard<std::mutex> lock_2(_task_queue_mutex, std::adopt_lock);
		...
		void resize(size_t threadpool_size)
		{
			if (_threadpool_size != threadpool_size)
			{
				...
				std::lock_guard<std::mutex> lock(_thread_list_mutex);
