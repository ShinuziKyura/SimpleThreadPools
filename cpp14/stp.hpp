...
namespace stp
{
	...
	public:
		...
		{
			value != task_priority::null ? _value = value, void() : void();
		}
	...
	public:
		...
		{
			priority != task_priority::null ? _task_priority = priority, void() : void();
		}
		void reset()
		{
	...
	private:
		...
		std::function<void()> _task_function{ std::bind(&task<RetType, ParamTypes ...>::_function<RetType>, this) };
		...
		template <>
		void _function<void>() // Alternative to if constexpr in _function()
		{
			_task_package();
			_task_state = task_state::ready;
		}
		...
		template <>
		RetType _result<void>() // Alternative to if constexpr in operator()()
		{
		}
		template <class ArgType>
		static auto _bind_forward(ArgType & arg) -> decltype(std::ref(arg))
		{
			return std::ref(arg);
		}
		template <class ArgType>
		static auto _bind_forward(ArgType && arg) -> decltype(std::bind(std::move<ArgType &>, std::ref(arg)))
		{
