...
// SimpleThreadPools - version B.3.11.0 - Static allocation of objects inside stp::task<T>
namespace stp
{
	...
	public:
		...
		[[nodiscard]] std::function<void()> prime() & // WARNING: The destructor for the task will block until operator() has been called for the object returned by this function
		{
			switch (_task_state.load(std::memory_order_relaxed))
			{
				case task_state::null:
			...
			return std::bind(&_task<RetType>::_function, this);
		}
	protected:
	...
	public:
		...
		task<RetType, ParamTypes ...>(RetType(* func)(ParamTypes ...),
									  ArgTypes && ... args) : _task<RetType>(func, std::forward<ArgTypes>(args) ...)
		{
		}
		template <class ObjType, class ... ArgTypes>
		task<RetType, ParamTypes ...>(RetType(ObjType::* func)(ParamTypes ...),
									  ObjType * obj,
									  ArgTypes && ... args) : _task<RetType>(func, obj, std::forward<ArgTypes>(args) ...)
		{
		}
		...
		template <class ... AnyParamTypes, std::enable_if_t<std::is_move_assignable_v<ResultType>, int> = 0>
		task<RetType, ParamTypes ...> & operator=(task<RetType, AnyParamTypes ...> && other) // Move assignment pseudo-operator
		{
		...
		std::add_lvalue_reference_t<RetType> get()
		{
			...
			if (this->_task_future.valid())
			{
				try
				{
					...
					{
						_task_result = std::move(this->_task_future.get());
					}
					else
					{
						_task_result = &this->_task_future.get();
					}
				}
				catch (...)
				{
	...
	public:
		...
		template <class ... ArgTypes>
		task<void, ParamTypes ...>(void(* func)(ParamTypes ...),
								   ArgTypes && ... args) : _task<void>(func, std::forward<ArgTypes>(args) ...)
		{
		}
		template <class ObjType, class ... ArgTypes>
		task<void, ParamTypes ...>(void(ObjType::* func)(ParamTypes ...),
								   ObjType * obj,
								   ArgTypes && ... args) : _task<void>(func, obj, std::forward<ArgTypes>(args) ...)
		{
		}
		template <class ... AutoParamTypes, class ... ArgTypes>
		task<void, ParamTypes ...>(void(* func)(AutoParamTypes ...),
								   ArgTypes && ... args) : _task<void>(func, std::forward<ArgTypes>(args) ...)
		{
		}
		template <class ObjType, class ... AutoParamTypes, class ... ArgTypes>
		task<void, ParamTypes ...>(void(ObjType::* func)(AutoParamTypes ...),
								   ObjType * obj,
								   ArgTypes && ... args) : _task<void>(func, obj, std::forward<ArgTypes>(args) ...)
		{
		}
	...
	inline task<RetType> make_task(RetType(* func)(ParamTypes ...), ArgTypes && ... args) // Allows disambiguation of function overload through specialization
	{
	...
	public:
		...
		template <class RetType, class ... ParamTypes>
		void execute(task<RetType, ParamTypes ...> & task, int_fast8_t priority)
		{
			auto function = task.prime();
			...
			_threadpool_task_queue.emplace(std::move(function), 
	...
	private:
		struct _task
		{
			_task(std::function<void()> && func = nullptr, uint_fast8_t prty = 0) :
				function(func),
				priority(prty)
			{
			}
